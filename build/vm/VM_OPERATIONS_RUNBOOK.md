...
- fullscreen entry/exit should invoke `ViewFullScreenMenuItem` by UI Automation; any keyboard fallback is posted to the product window handle with `PostMessage` and then verified by the harness wait loop; do not use global `SendKeys` for product fullscreen toggles
