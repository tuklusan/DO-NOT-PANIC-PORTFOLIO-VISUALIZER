...
if (Test-Path $thirdPartyNotices) {
    Copy-Item -LiteralPath $thirdPartyNotices -Destination (Join-Path $payloadRoot "THIRD-PARTY-NOTICES.md") -Force
}

$thirdPartyLicenses = Join-Path $root "THIRD-PARTY-LICENSES"
if (Test-Path $thirdPartyLicenses) {
    Copy-Item -LiteralPath $thirdPartyLicenses -Destination (Join-Path $payloadRoot "THIRD-PARTY-LICENSES") -Recurse -Force
}
