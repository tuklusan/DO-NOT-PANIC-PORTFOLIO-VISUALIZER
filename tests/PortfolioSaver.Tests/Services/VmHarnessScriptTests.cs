...
public sealed class VmHarnessScriptTests
{
    ...
    public void GuestUxDeepExercise_LogsAndValidatesPhaseAndVersionChecks()
    {
        ...
        Assert.Contains("Send-KeySequence -Keys @('{PGDN}','{PGDN}')", script, StringComparison.Ordinal);
    ...
    public void GuestUxDeepExercise_SupportsSshWorkspaceRoots_AndWritesLocalTraceBundles()
    {
        ...
        Assert.Contains("public struct POINTL", script, StringComparison.Ordinal);
        Assert.Contains("mode-not-supported", script, StringComparison.Ordinal);
        Assert.Contains("Start-Process -FilePath 'explorer.exe' -ArgumentList 'ms-settings:display' -PassThru", script, StringComparison.Ordinal);
        ...
        Assert.Contains("$summary.SupportedDisplayModes = @($displayApply.AvailableModes", script, StringComparison.Ordinal);
