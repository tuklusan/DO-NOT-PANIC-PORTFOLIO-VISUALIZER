...
public sealed class VmHarnessScriptTests
{
    ...
    public void GuestUxDeepExercise_SupportsSshWorkspaceRoots_AndWritesLocalTraceBundles()
    {
        ...
        Assert.Contains("$summary.DesktopVersionCheck = \"Passed\"", script, StringComparison.Ordinal);
        ...
        Assert.Contains("if ($nextCaptureAt -ge $captureDeadline)", script, StringComparison.Ordinal);
        int captureSleepStart = script.IndexOf("$nextCaptureAt = $frameStartedAt.AddSeconds($effectiveCaptureIntervalSeconds)", StringComparison.Ordinal);
        int captureSleepEnd = script.IndexOf("$i++", captureSleepStart, StringComparison.Ordinal);
